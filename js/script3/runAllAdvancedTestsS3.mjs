...
async function runHeisenbugReproStrategy_TypedArrayVictim() {
    ...
    // ... (lógica de processamento de resultado como na R14/R17, adaptando logs para R18) ...
    ...
    else if(result){logS3(` RUNNER R18: Concluído.`,"good",FNAME_RUNNER);logS3(` RUNNER R18: Melhor OOB: ${result.oob_value_of_best_result||'N/A'}`,"info",FNAME_RUNNER);logS3(` RUNNER R18: Sonda TC: ${result.tc_probe_details?JSON.stringify(result.tc_probe_details):'N/A'}`,"leak",FNAME_RUNNER);
    ...
    logS3(` RUNNER R18: TC Sonda: ${tcOK?"CONFIRMADA":"NÃO"}`,"vuln",FNAME_RUNNER);
    ...
    if(getterAddr)logS3(` RUNNER R18: Addrof Getter TC: ${getterAddr.msg} (Val: ${getterAddr.value})`,getterAddr.success?"vuln":"warn",FNAME_RUNNER);
    if(directAddr)logS3(` RUNNER R18: Addrof Direct TC: ${directAddr.msg} (Val: ${JSON.stringify(directAddr.value)})`,directAddr.success?"vuln":"warn",FNAME_RUNNER);
