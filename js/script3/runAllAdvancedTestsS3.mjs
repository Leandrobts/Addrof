// js/script3/runAllAdvancedTestsS3.mjs (Runner para R45 - addrof via Heap Spray)
import { logS3, PAUSE_S3, MEDIUM_PAUSE_S3 } from './s3_utils.mjs';
import { getOutputAdvancedS3, getRunBtnAdvancedS3 } from '../dom_elements.mjs';
import {
    executeTypedArrayVictimAddrofAndWebKitLeak_R45 as executeTest, // Renomear para clareza
    FNAME_MODULE_TYPEDARRAY_ADDROF_V82_AGL_R45_WEBKIT as FNAME_MODULE
} from './testArrayBufferVictimCrash.mjs';

async function runHeisenbugReproStrategy_SprayAddrof_R45() {
    const FNAME_RUNNER = "runHeisenbugReproStrategy_SprayAddrof_R45";
    ...
    if (!result) {
        ...
        logS3(`  RUNNER R45(SprayAddrof): Objeto de resultado inválido ou nulo recebido do teste.`, "critical", FNAME_RUNNER);
        return;
    }
    ...
    if (result.webkit_leak_result.success) {
        ...
        logS3(`    Falha no WebKit Leak: ${result.webkit_leak_result.msg}`, "warn", FNAME_RUNNER);
...
export async function runAllAdvancedTestsS3() {
    const FNAME_ORCHESTRATOR = `${FNAME_MODULE}_MainOrchestrator_SprayAddrof`;
    logS3(`==== INICIANDO Script 3 R45_SprayAddrof (${FNAME_ORCHESTRATOR}) ... ====`, 'test', FNAME_ORCHESTRATOR);
    
    await runHeisenbugReproStrategy_SprayAddrof_R45();
    
    logS3(`\n==== Script 3 R45_SprayAddrof (${FNAME_ORCHESTRATOR}) CONCLUÍDO ====`, 'test', FNAME_ORCHESTRATOR);
