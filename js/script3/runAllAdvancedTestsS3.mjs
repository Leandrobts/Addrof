...
import {
    executeTypedArrayVictimAddrofTest_CorrectedV63_FuzzFlow, // ATUALIZADO para v64
    FNAME_MODULE_TYPEDARRAY_ADDROF_V64_CFRF // ATUALIZADO para v64
} from './testArrayBufferVictimCrash.mjs';

async function runHeisenbugReproStrategy_TypedArrayVictim() {
    const FNAME_RUNNER = "runHeisenbugReproStrategy_TypedArrayVictim_CorrectedV63_FuzzFlow";
    logS3(`==== INICIANDO Estratégia de Reprodução do Heisenbug (${FNAME_RUNNER}) ====`, 'test', FNAME_RUNNER);

    const result = await executeTypedArrayVictimAddrofTest_CorrectedV63_FuzzFlow();
    ...
    if (result.errorCapturedMain) {
        ...
        document.title = `Heisenbug (TypedArray-CFRF) ERR: ${result.errorCapturedMain.name}`;
    } else {
...
export async function runAllAdvancedTestsS3() {
    const FNAME_ORCHESTRATOR = `${FNAME_MODULE_TYPEDARRAY_ADDROF_V64_CFRF}_MainOrchestrator`;
    // ... (resto da função runAllAdvancedTestsS3 idêntica à v63, apenas com FNAME_MODULE_TYPEDARRAY_ADDROF_V64_CFRF)
    ...
    logS3(`==== INICIANDO Script 3 (${FNAME_ORCHESTRATOR}): Reproduzindo Heisenbug com TypedArray Vítima (CorrectedV63_FuzzFlow) ====`, 'test', FNAME_ORCHESTRATOR);
    ...
    if (document.title.startsWith("Iniciando") || document.title.includes(FNAME_MODULE_TYPEDARRAY_ADDROF_V64_CFRF)) {
        if (!document.title.includes("CRASH") && !document.title.includes("RangeError") &&
            ...
            document.title = `${FNAME_MODULE_TYPEDARRAY_ADDROF_V64_CFRF} Concluído`;
        }
    }
}
