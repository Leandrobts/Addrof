// js/script3/runAllAdvancedTestsS3.mjs (ATUALIZADO para Revisado 66 - WebKit Leak com Logs Verbosos)
...
async function runHeisenbugReproStrategy_TypedArrayVictim_R43() {
    const FNAME_RUNNER = "runHeisenbugReproStrategy_TypedArrayVictim_R66"; // Nome do Runner atualizado para R66
    ...
    if (result.errorOccurred) {
        logS3(`  RUNNER R66: Teste principal capturou ERRO: ${String(result.errorOccurred)}`, "critical", FNAME_RUNNER);
        document.title = `${module_name_for_title}: MainTest ERR!`;
    } else if (result) {
        logS3(`  RUNNER R66: Teste de L/E completou.`, "good", FNAME_RUNNER);
        const webkitLeakResult = result.webkit_leak_result;

        if (webkitLeakResult) {
            logS3(`  RUNNER R66: Resultado do Vazamento da Base WebKit: ${webkitLeakResult.msg}`, webkitLeakResult.success ? "vuln" : "warn", FNAME_RUNNER);
            if(webkitLeakResult.success) {
                logS3(`  RUNNER R66: Endereço Base da WebKit Encontrado: ${webkitLeakResult.webkit_base_candidate}`, "vuln", FNAME_RUNNER);
                document.title = `${module_name_for_title}: WebKit Base Leaked!`;
            } else {
                 document.title = `${module_name_for_title}: WebKit Leak FAILED`;
            }
        } else {
            logS3(`  RUNNER R66: Teste WebKit Base Leak não produziu resultado.`, "warn", FNAME_RUNNER);
...
export async function runAllAdvancedTestsS3() {
    ...
    logS3(`==== INICIANDO Script 3 R66 (${FNAME_ORCHESTRATOR}) ... ====`, 'test', FNAME_ORCHESTRATOR);
    
    await testJITBehavior();
    await PAUSE_S3(500);
    
    await runHeisenbugReproStrategy_TypedArrayVictim_R43();
    logS3(`\n==== Script 3 R66 (${FNAME_ORCHESTRATOR}) CONCLUÍDO ====`, 'test', FNAME_ORCHESTRATOR);
