...
import { 
    executeTypedArrayVictimAddrofTest_ConsistentLogging,   // NOME DA FUNÇÃO ATUALIZADO
    FNAME_MODULE_TYPEDARRAY_ADDROF_V13_CL    // NOME DO MÓDULO ATUALIZADO
} from './testArrayBufferVictimCrash.mjs';

async function runHeisenbugReproStrategy_TypedArrayVictim() {
    const FNAME_RUNNER = "runHeisenbugReproStrategy_TypedArrayVictim_ConsistentLogging";
    logS3(`==== INICIANDO Estratégia de Reprodução do Heisenbug com TypedArray Vítima (ConsistentLogging) ====`, 'test', FNAME_RUNNER);

    const result = await executeTypedArrayVictimAddrofTest_ConsistentLogging(); 

    logS3(`   Total de chamadas da sonda toJSON durante o teste: ${result.total_probe_calls}`, "info", FNAME_RUNNER);
    ...
    if (result.errorOccurred) {
        ...
        if (result.errorOccurred.name === 'RangeError') {
            ...
            document.title = `Heisenbug (TypedArray-CL) RangeError!`; 
         } else {
            document.title = `Heisenbug (TypedArray-CL) ERR: ${result.errorOccurred.name}`;
         }
         ...
         if (!document.title.includes("CONGELOU")) document.title = `Heisenbug (TypedArray-CL) StackOverflow?`;
    } else {
        ...
        if (result.toJSON_details && result.toJSON_details.error_in_toJSON) {
            ...
            document.title = `Heisenbug (TypedArray-CL) toJSON_ERR`;
            ...
            if (result.toJSON_details.this_is_victim_ref !== null) { 
                logS3(`       Na última sonda confusa, 'this' === victim_typed_array_ref_v13? ${result.toJSON_details.this_is_victim_ref}`, "info");
            }
            if (result.toJSON_details.this_is_prev_probe_return !== null) {
                 logS3(`       Na última sonda confusa, 'this' era o retorno da sonda anterior? ${result.toJSON_details.this_is_prev_probe_return}`, "info");
            }
            ...
            if (!document.title.includes("SUCESSO") && !document.title.includes("Addr Falhou")) {
                 document.title = `Heisenbug (TypedArray-CL) Sonda OK, Addr Falhou`;
            }
        } else if (document.title.startsWith("Iniciando") || document.title.includes(FNAME_MODULE_TYPEDARRAY_ADDROF_V13_CL) || document.title.includes("Probing")) {
            if (!document.title.includes("SUCESSO") && !document.title.includes("Addr Falhou")) {
                 document.title = `Heisenbug (TypedArray-CL) Test OK`;
            }
        }
    }
    ...
    logS3(`==== Estratégia de Reprodução do Heisenbug (ConsistentLogging) CONCLUÍDA ====`, 'test', FNAME_RUNNER);
}

export async function runAllAdvancedTestsS3() {
    const FNAME_ORCHESTRATOR = `${FNAME_MODULE_TYPEDARRAY_ADDROF_V13_CL}_MainOrchestrator`; 
    ...
    logS3(`==== INICIANDO Script 3 (${FNAME_ORCHESTRATOR}): Reproduzindo Heisenbug com TypedArray Vítima (ConsistentLogging) ====`, 'test', FNAME_ORCHESTRATOR);
    ...
    if (document.title.startsWith("Iniciando") || document.title.includes(FNAME_MODULE_TYPEDARRAY_ADDROF_V13_CL)) {
        if (!document.title.includes("CRASH") && !document.title.includes("RangeError") && 
            !document.title.includes("SUCESSO") && !document.title.includes("Addr Falhou") && 
            !document.title.includes("ERR") && !document.title.includes("Sonda OK")) { // Ajustado para não sobrescrever "Sonda OK"
            document.title = `${FNAME_MODULE_TYPEDARRAY_ADDROF_V13_CL} Concluído`;
        }
    }
}
