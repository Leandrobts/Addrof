// js/script3/runAllAdvancedTestsS3.mjs (Runner para Base Melhorada Corrigido)
...
async function runHeisenbugReproStrategy_TypedArrayVictim_R43_ImprovedBaseCorrected() {
    const FNAME_RUNNER = "runHeisenbugReproStrategy_TypedArrayVictim_R43_IBCorrected"; // Nome atualizado
    ...
    if (result.errorOccurred && !result.oob_params_of_best_result) {
        logS3(`  RUNNER R43(IBCorrected): Teste principal capturou ERRO: ${String(result.errorOccurred)}`, "critical", FNAME_RUNNER);
        document.title = `${module_name_for_title}: MainTest ERR!`;
    } else if (result) {
        logS3(`  RUNNER R43(IBCorrected): Teste principal completado. Analisando resultados...`, "good", FNAME_RUNNER);

        logS3(`  --- MELHOR RESULTADO GERAL ENCONTRADO ---`, "info_emphasis", FNAME_RUNNER);
        let bestParamsMsg = 'N/A';
        if (result.oob_params_of_best_result) { // Este campo agora é um objeto
        ...
        // Usar heisenbug_on_M2_confirmed_by_tc_probe do objeto result principal
        ...
        // Logar resultados globais de addrof e webkitleak
        ...
        if (result.errorOccurred && result.oob_params_of_best_result) { // Erro associado ao "melhor" resultado (ex: todas iterações falharam com erro)
        ...
        if (result.iteration_results_summary && result.iteration_results_summary.length > 0) {
            ...
            result.iteration_results_summary.forEach((iter_sum, index) => {
                ...
                else { logMsg += "Sem Sucesso Notável"; } // Alterado para mais clareza
                if(iter_sum.error) logMsg += `, Err: ${iter_sum.error}`;
                
                let logLevel = "info";
                if (iter_sum.webkit_leak_success_this_iter) logLevel = "success_major";
                else if (iter_sum.addrof_success_this_iter) logLevel = "vuln_potential"; // Um addrof na iteração é muito bom
        ...
        document.title = `${module_name_for_title}_R43_IBCorr: Invalid Result Obj`;
        logS3(`  RUNNER R43(IBCorrected): Objeto de resultado inválido ou nulo.`, "critical", FNAME_RUNNER);
    }
...
export async function runAllAdvancedTestsS3() {
    const FNAME_ORCHESTRATOR = `${FNAME_MODULE_TYPEDARRAY_ADDROF_V82_AGL_R43_WEBKIT}_MainOrchestrator_IBCorr`;
    logS3(`==== INICIANDO Script 3 R43_IBCorr (${FNAME_ORCHESTRATOR}) ... ====`, 'test', FNAME_ORCHESTRATOR);
    
    await runHeisenbugReproStrategy_TypedArrayVictim_R43_ImprovedBaseCorrected();
    
    logS3(`\n==== Script 3 R43_IBCorr (${FNAME_ORCHESTRATOR}) CONCLUÍDO ====`, 'test', FNAME_ORCHESTRATOR);
    ...
    if (document.title.includes(FNAME_MODULE_TYPEDARRAY_ADDROF_V82_AGL_R43_WEBKIT) &&
        ...
        document.title = `${FNAME_MODULE_TYPEDARRAY_ADDROF_V82_AGL_R43_WEBKIT}_R43_IBCorr_Done`;
    }
}
