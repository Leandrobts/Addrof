...
import { 
    executeTypedArrayVictimAddrofTest_LeakFromCapturedDetails,   // NOME DA FUNÇÃO ATUALIZADO
    FNAME_MODULE_TYPEDARRAY_ADDROF_V33_LFCD    // NOME DO MÓDULO ATUALIZADO
} from './testArrayBufferVictimCrash.mjs';

async function runHeisenbugReproStrategy_TypedArrayVictim() {
    const FNAME_RUNNER = "runHeisenbugReproStrategy_TypedArrayVictim_LeakFromCapturedDetails";
    logS3(`==== INICIANDO Estratégia de Reprodução do Heisenbug com TypedArray Vítima (LeakFromCapturedDetails) ====`, 'test', FNAME_RUNNER);

    const result = await executeTypedArrayVictimAddrofTest_LeakFromCapturedDetails(); 
    ...
    if (result.errorOccurred) {
        ...
        document.title = `Heisenbug (TypedArray-LFCD) ERR: ${result.errorOccurred.name}`;
    } else {
        logS3(`   RESULTADO: Completou. Detalhes da sonda de interesse (onde TC e escritas ocorreram): ${result.toJSON_details ? JSON.stringify(result.toJSON_details) : 'N/A'}`, "good", FNAME_RUNNER);
        ...
        if (result.toJSON_details && 
            ...
            result.toJSON_details.leaky_A_payload_assigned) { // Verifica se as escritas foram marcadas como feitas
        ...
        if (result.toJSON_details && result.toJSON_details.error_in_probe) {
            ...
            document.title = `Heisenbug (TypedArray-LFCD) toJSON_ERR`;
            ...
            if (!anyAddrofSuccess) {
                 document.title = `Heisenbug (TypedArray-LFCD) TC&Write OK, Addr Fail`;
            } else { 
                 document.title = `Heisenbug (TypedArray-LFCD) AddrFromDetails SUCCESS!`;
            }
        } else if (document.title.startsWith("Iniciando") || document.title.includes(FNAME_MODULE_TYPEDARRAY_ADDROF_V33_LFCD) || document.title.includes("Probing")) {
            if (!anyAddrofSuccess) {
                 document.title = `Heisenbug (TypedArray-LFCD) Test OK`;
            }
        }
    }
    ...
    logS3(`==== Estratégia de Reprodução do Heisenbug (LeakFromCapturedDetails) CONCLUÍDA ====`, 'test', FNAME_RUNNER);
}

export async function runAllAdvancedTestsS3() {
    const FNAME_ORCHESTRATOR = `${FNAME_MODULE_TYPEDARRAY_ADDROF_V33_LFCD}_MainOrchestrator`; 
    ...
    logS3(`==== INICIANDO Script 3 (${FNAME_ORCHESTRATOR}): Reproduzindo Heisenbug com TypedArray Vítima (LeakFromCapturedDetails) ====`, 'test', FNAME_ORCHESTRATOR);
    ...
    if (document.title.startsWith("Iniciando") || document.title.includes(FNAME_MODULE_TYPEDARRAY_ADDROF_V33_LFCD)) {
        if (!document.title.includes("CRASH") && !document.title.includes("RangeError") && 
            ...
            document.title = `${FNAME_MODULE_TYPEDARRAY_ADDROF_V33_LFCD} Concluído`;
        }
    }
}
