// js/script3/runAllAdvancedTestsS3.mjs (Runner para Testes Massivos Finais Revisados)
...
async function runHeisenbugReproStrategy_TypedArrayVictim_R43_MassiveFinal() {
    const FNAME_RUNNER = "runHeisenbugReproStrategy_TypedArrayVictim_R43_MassiveFinal";
    ...
    if (result.errorOccurred && !result.oob_params_of_best_result) {
        logS3(`  RUNNER R43(MassiveFinal): Teste principal capturou ERRO: ${String(result.errorOccurred)}`, "critical", FNAME_RUNNER);
        document.title = `${module_name_for_title}: MainTest ERR!`;
    } else if (result) {
        logS3(`  RUNNER R43(MassiveFinal): Teste principal completado. Analisando resultados...`, "good", FNAME_RUNNER);
        ...
        const heisenbugInBest = result.heisenbug_on_M2_confirmed_by_tc_probe; // Do objeto resultado geral
        ...
        if (result.best_victim_analysis) {
            ...
            if (va.found_pointers_in_victim_ta && va.found_pointers_in_victim_ta.length > 0) {
                ...
                va.found_pointers_in_victim_ta.slice(0, 5).forEach(p => { // Logar os primeiros 5
        ...
        if (result.iteration_results_summary && result.iteration_results_summary.length > 0) {
            ...
            result.iteration_results_summary.forEach((iter_sum, index) => {
                ...
                if (iter_sum.addrof_success_this_iter) highlights.push("ADDROF_GenOK"); // Se houver um campo genérico
                ...
                let logLevel = "info";
                if (iter_sum.addrof_success_this_iter || iter_sum.victim_ta_analysis?.addrof_achieved_via_victim_corruption) logLevel = "success_major";
        ...
        document.title = `${module_name_for_title}_R43_MassiveFinal: Invalid Result Obj`;
        logS3(`  RUNNER R43(MassiveFinal): Objeto de resultado inválido ou nulo recebido do teste.`, "critical", FNAME_RUNNER);
    }
...
export async function runAllAdvancedTestsS3() {
    const FNAME_ORCHESTRATOR = `${FNAME_MODULE_TYPEDARRAY_ADDROF_V82_AGL_R43_WEBKIT}_MainOrchestrator_MassiveFinal`;
    logS3(`==== INICIANDO Script 3 R43_MassiveFinal (${FNAME_ORCHESTRATOR}) ... ====`, 'test', FNAME_ORCHESTRATOR);
    
    await runHeisenbugReproStrategy_TypedArrayVictim_R43_MassiveFinal();
    
    logS3(`\n==== Script 3 R43_MassiveFinal (${FNAME_ORCHESTRATOR}) CONCLUÍDO ====`, 'test', FNAME_ORCHESTRATOR);
    ...
    if (document.title.includes(FNAME_MODULE_TYPEDARRAY_ADDROF_V82_AGL_R43_WEBKIT) &&
        ...
        document.title = `${FNAME_MODULE_TYPEDARRAY_ADDROF_V82_AGL_R43_WEBKIT}_R43_MassiveFinal_Done`;
    }
}
