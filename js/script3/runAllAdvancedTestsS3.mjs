...
import {
    executeTypedArrayVictimAddrofTest_SelfRefThenLeak, // ATUALIZADO para v54
    FNAME_MODULE_TYPEDARRAY_ADDROF_V54_SRTL // ATUALIZADO para v54
...
async function runHeisenbugReproStrategy_TypedArrayVictim() {
    ...
    if (result.errorCapturedMain) {
        ...
        if(heisenbugOnC1){
            ...
            logS3(`  ALERT: Heisenbug no C1_details (Call #2) ou modificação NÃO CONFIRMADA PELA SONDA.`, "error", FNAME_RUNNER);
        }
        ...
        if (anyAddrofSuccess) {
            ...
            // Se a heisenbug foi confirmada OU se houve erro de circularidade (que implica controle)
            document.title = `${FNAME_MODULE_TYPEDARRAY_ADDROF_V54_SRTL}: Heisenbug OK, Addr Fail`;
        } else {
            document.title = `${FNAME_MODULE_TYPEDARRAY_ADDROF_V54_SRTL}: No Heisenbug?`;
        }
    }
...
export async function runAllAdvancedTestsS3() {
    const FNAME_ORCHESTRATOR = `${FNAME_MODULE_TYPEDARRAY_ADDROF_V54_SRTL}_MainOrchestrator`;
    ...
    if (document.title.startsWith("Iniciando") || document.title.includes(FNAME_MODULE_TYPEDARRAY_ADDROF_V54_SRTL)) {
        if (!document.title.includes("CRASH") && !document.title.includes("RangeError") &&
            !document.title.includes("SUCCESS") && !document.title.includes("Addr Fail") &&
            !document.title.includes("ERR") && !document.title.includes("Heisenbug OK")) { // Ajustado para Heisenbug OK
            document.title = `${FNAME_MODULE_TYPEDARRAY_ADDROF_V54_SRTL} Concluído`;
        }
    }
}
