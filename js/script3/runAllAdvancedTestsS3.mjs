...
import {
    executeTypedArrayVictimAddrofTest_FuzzDataPointerOffsets, // NOME DA FUNÇÃO ATUALIZADO
    FNAME_MODULE_TYPEDARRAY_ADDROF_V44_FDPO // NOME DO MÓDULO ATUALIZADO
} from './testArrayBufferVictimCrash.mjs';

async function runHeisenbugReproStrategy_TypedArrayVictim() {
    const FNAME_RUNNER = "runHeisenbugReproStrategy_TypedArrayVictim_FuzzDataPointerOffsets";
    logS3(`==== INICIANDO Estratégia de Reprodução do Heisenbug (FuzzDataPointerOffsets) ====`, 'test', FNAME_RUNNER);

    const result = await executeTypedArrayVictimAddrofTest_FuzzDataPointerOffsets();
    ...
    if (result.errorOccurred) {
        ...
        document.title = `Heisenbug (TypedArray-FDPO) ERR: ${result.errorOccurred.name}`;
    ...
    logS3(`==== Estratégia de Reprodução do Heisenbug (FuzzDataPointerOffsets) CONCLUÍDA ====`, 'test', FNAME_RUNNER);
}

export async function runAllAdvancedTestsS3() {
    const FNAME_ORCHESTRATOR = `${FNAME_MODULE_TYPEDARRAY_ADDROF_V44_FDPO}_MainOrchestrator`;
    ...
    logS3(`==== INICIANDO Script 3 (${FNAME_ORCHESTRATOR}): Reproduzindo Heisenbug com TypedArray Vítima (FuzzDataPointerOffsets) ====`, 'test', FNAME_ORCHESTRATOR);
    ...
    if (document.title.startsWith("Iniciando") || document.title.includes(FNAME_MODULE_TYPEDARRAY_ADDROF_V44_FDPO)) {
        if (!document.title.includes("CRASH") && !document.title.includes("RangeError") &&
            ...
            document.title = `${FNAME_MODULE_TYPEDARRAY_ADDROF_V44_FDPO} Concluído`;
        }
    }
}
