// js/script3/runAllAdvancedTestsS3.mjs (ATUALIZADO para Revisado 9)
import { logS3, PAUSE_S3, MEDIUM_PAUSE_S3 } from './s3_utils.mjs';
import { getOutputAdvancedS3, getRunBtnAdvancedS3 } from '../dom_elements.mjs';
import {
    executeTypedArrayVictimAddrofTest_AdvancedGetterLeak_R9, // <<<< NOME DA FUNÇÃO ATUALIZADO
    FNAME_MODULE_TYPEDARRAY_ADDROF_V82_AGL
} from './testArrayBufferVictimCrash.mjs';

async function runHeisenbugReproStrategy_TypedArrayVictim() {
    const FNAME_RUNNER = "runHeisenbugReproStrategy_TypedArrayVictim_R9"; // Atualizado
    logS3(`==== INICIANDO Estratégia de Reprodução do Heisenbug (${FNAME_RUNNER}) ====`, 'test', FNAME_RUNNER);

    const result = await executeTypedArrayVictimAddrofTest_AdvancedGetterLeak_R9(); // <<<< NOME DA FUNÇÃO ATUALIZADO

    if (result.errorOccurred) {
        logS3(`  RUNNER: Teste principal capturou ERRO: ${result.errorOccurred}.`, "critical", FNAME_RUNNER);
        document.title = `${FNAME_MODULE_TYPEDARRAY_ADDROF_V82_AGL}_R9: MainTest ERR!`;
    } else if (result) {
        // ... (lógica de logging de resultado mantida da Revisado 8, já deve estar OK)
        ...
        if (addrofCoreSuccess) {
            document.title = `${FNAME_MODULE_TYPEDARRAY_ADDROF_V82_AGL}_R9: AddrofCore SUCCESS! (OOB: ${result.oob_value_of_best_result})`;
        } else if (heisenbugSuccessfullyDetected) {
            document.title = `${FNAME_MODULE_TYPEDARRAY_ADDROF_V82_AGL}_R9: TC OK, Addrof Fail (OOB: ${result.oob_value_of_best_result})`;
        } else {
            document.title = `${FNAME_MODULE_TYPEDARRAY_ADDROF_V82_AGL}_R9: No TC Confirmed (OOB: ${result.oob_value_of_best_result})`;
        }
        
        if(result.iteration_results_summary && result.iteration_results_summary.length > 0){
            logS3(`  RUNNER: Sumário completo das iterações (R9):`, "info", FNAME_RUNNER);
        ...
        document.title = `${FNAME_MODULE_TYPEDARRAY_ADDROF_V82_AGL}_R9: Invalid Result Obj`;
    }
    // ... (resto como antes) ...
...
export async function runAllAdvancedTestsS3() {
    const FNAME_ORCHESTRATOR = `${FNAME_MODULE_TYPEDARRAY_ADDROF_V82_AGL}_MainOrchestrator_R9`; // Atualizado
    ...
    if (document.title.includes(FNAME_MODULE_TYPEDARRAY_ADDROF_V82_AGL) && 
        ...
        document.title = `${FNAME_MODULE_TYPEDARRAY_ADDROF_V82_AGL}_R9 Run Finished`; // Atualizado
    }
}
